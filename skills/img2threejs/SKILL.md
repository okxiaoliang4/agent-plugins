---
name: img2threejs
description: Turn an object or character reference image into a quality-gated, animation-ready procedural Three.js model built
  in code. Use when reconstruction must remain code-only or needs deterministic parametric geometry, exact runtime hierarchy,
  physically meaningful deformation, staged sculpt specs, or generated-mesh services are unsuitable; prefer a generated-mesh
  workflow for ordinary visually matched product assets when external generation is allowed.
license: Apache-2.0
---
...
Do not use this as the automatic first route for every image-to-3D request. When external generated
assets are allowed and the goal is a visually plausible marketing, AR, game, or web product model,
prefer the `imagegen` + `tripo-cli` workflow and apply its geometry, parts, material, and runtime
gates. Return here when the user requires code-only delivery, deterministic parameterization,
measured or physically meaningful shape changes, exact authored hierarchy/topology, or when the
generated mesh fails an identity-defining gate. A hybrid is valid: use Tripo for broad product form
and procedural Three.js for critical geometry such as prescription lenses, measurements, or
